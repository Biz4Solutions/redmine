class Timesheet < ApplicationRecord
  ...
  # Returns true if the timesheet can be resubmitted (is rejected)
  def can_resubmit?
    status == STATUS_REJECTED && time_entries.any?
  end

  # Returns true if the timesheet can be edited
  def can_edit?
    status == STATUS_DRAFT || status == STATUS_REJECTED
  end
  ...
  # Submit the timesheet for approval (handles both initial submission and resubmission)
  def submit
    return false unless can_submit? || can_resubmit?

    self.status = STATUS_PENDING
    # Clear rejection information when resubmitting
    if status_was == STATUS_REJECTED
      self.approved_by_id = nil
      self.approved_on = nil
      self.rejection_reason = nil

      # Reset time entries to pending status
      time_entries.each do |entry|
        entry.status = TimeEntry::STATUS_PENDING
        entry.approved_by_id = nil
        entry.approved_on = nil
        entry.rejection_reason = nil
